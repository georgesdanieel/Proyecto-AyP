...
def caracteristicas_naves(starship_list):
    ...
    axs[0, 0].set_yticks(np.arange(0, df["Length"].max() * 1.1, 100)) #me permite organizar los valores de menor a mayor
    ...
    axs[0, 1].set_yticks(np.arange(0, df["Cargo Capacity"].max() * 1.1, 1000))
    ...
    axs[1, 0].set_yticks(np.arange(0, df["Hyperdrive Rating"].max() * 1.1, 0.5))
    ...
    axs[1, 1].set_yticks(np.arange(0, df["MGLT"].max() * 1.1, 10))
